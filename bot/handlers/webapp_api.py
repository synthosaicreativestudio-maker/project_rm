...
async def handle_generate(request):
    try:
        ...
        await bot.send_message(chat_id=user_id, text=f"✅ Задача получена: {action_type.upper()}\nПромт: {prompt[:50]}...")

        # Run generation in background
        import asyncio
        asyncio.create_task(process_generation(bot, user_id, action_type, prompt, params))
        
        return web.json_response({'status': 'success', 'message': 'Task started'})
    except Exception as e:
        logger.error(f"Error in handle_generate: {e}")
        return web.json_response({'status': 'error', 'message': str(e)}, status=500)

async def process_generation(bot, user_id, action_type, prompt, params):
    try:
        if action_type == 'image':
            ...
            await bot.send_message(chat_id=user_id, text=f"🎨 {model_id} рисует...")
            ...
            if image_bytes:
                ...
                await bot.send_photo(chat_id=user_id, photo=photo_file, caption=f"✨ Generated by {model_id}")
            else:
            ...
            await bot.send_message(chat_id=user_id, text=f"🎥 {model_id} начала рендеринг...")
            
            video_uri = await veo_service.generate_video(prompt)
            
            if video_uri:
                 await bot.send_message(chat_id=user_id, text=f"🎬 Готово! Результат: {video_uri}")
            else:
                 await bot.send_message(chat_id=user_id, text="❌ Не удалось сгенерировать видео (возможно, превышена квота).")

    except Exception as e:
        logger.error(f"Error in process_generation: {e}")
        await bot.send_message(chat_id=user_id, text=f"❌ Ошибка генерации: {str(e)}")

def setup_web_routes(app, bot):
    app['bot'] = bot
